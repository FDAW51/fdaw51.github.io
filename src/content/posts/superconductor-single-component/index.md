...
description: "系统介绍超导现象的两大理论支柱：微观的 BCS 理论（Cooper 对、电子-声子耦合、能隙）与宏观的 Ginzburg-Landau 理论（序参量、自由能最小化、两类超导体与磁通涡旋）。"
tags: ["BCS Theory", "Cooper Pair", "Ginzburg-Landau Theory", "Meissner Effect", "Coherence Length", "Penetration Depth", "Flux Quantization", "Magnetic Flux Vortex", "Superconductivity"]
category: 凝聚态物理/超导
draft: false
---
...
$$
然后两边乘以 $e^{-i\mathbf{q(r_1-r_2)}}$并对整个空间积分（位置空间）：
...
GL 理论对于超导现象给出了宏观描述，它引入了一个序参量 $\psi(\mathbf{r})$,它的模方代表在超导体中某一个位置的Cooper- Pair的密度  $n_s(\mathbf{r})$:
$$
...
$$
左边根据波函数的单值性，为 $2n\pi$，右边根据Stokes 定理及（$\nabla \times A = B$）：
$$
...
- [[BCS Theory]] — 电子-声子耦合导致 Cooper 对形成的微观超导理论（1957）
- [[Cooper Pair]] — 费米面附近两个电子通过晶格畸变形成的束缚态
- [[Ginzburg-Landau Theory]] — 基于序参量与自由能极小化的超导唯象理论（1950）
- [[Meissner Effect]] — 超导体对磁场的完全排斥，证明超导是热力学相
- [[Coherence Length]] — 超导序参量受扰动后恢复的特征长度 $\xi$
- [[Penetration Depth]] — 外磁场在超导体内部的衰减特征长度 $\lambda$
- [[Type I and Type II Superconductors]] — 由 GL 参数 $\kappa = \lambda/\xi$ 区分的两类超导体
- [[Magnetic Flux Vortex]] — II 类超导体中磁通穿透的量子化涡旋结构
- [[Flux Quantization]] — 磁通量子 $\Phi_0 = hc/2e$，验证 Cooper 对电荷 $2e$

> 参见综合页：[[synthesis/BCS 理论与 GL 理论对比]] — 微观 vs 宏观超导理论的互补关系
